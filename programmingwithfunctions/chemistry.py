# from test_chemistry_1 import SYMBOL_INDEX
...
def main():
    ...
    chemical = input('What is the chemical formula that you are looking for? ')
    grams = float(input('What is the mass in grams? '))
    
    table = make_periodic_table()

    formula = parse_formula(chemical, table)

    mass = compute_molar_mass(formula, table)
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    atomic_mass = 0
    for i in range(0,len(symbol_quantity_list)):
        symbol = symbol_quantity_list[i][0]
        value = periodic_table[symbol][1]
        quantity = symbol_quantity_list[i][1]

        atomic_mass += value * quantity

    # for periodic_table in symbol_quantity_list.items():
    #     symbol = symbol_quantity_list[NAME_INDEX]
    #     quantity = symbol_quantity_list[ATOMIC_MASS_INDEX]
    #     atomic_mass = symbol_quantity_list[ATOMIC_MASS_INDEX]
